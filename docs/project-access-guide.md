...
5. Open Redis subscription on `risk.alerts.live`
...
4. No alerts in UI: check `risk.events.queue` and `risk.alerts.queue` message counts in RabbitMQ.
...
6. `backend/services/api_gateway/app/api/routes_auth.py`
7. `backend/services/notification_service/app/api/routes.py`
